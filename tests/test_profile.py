...
class TestProfile:
    ...
    def test_profile_page(self, mozwebqa):
        ...
        Assert.true(profile_page.is_manage_api_keys_visible)
        Assert.true(profile_page.is_submit_demo_button_visible)
        profile_page.click_manage_api_keys()
        key_page = profile_page.KeyPage(mozwebqa)
        Assert.true(key_page.create_key)
        Assert.equal(key_page.delete_latest_key,key_page.base_url+"/en-US/keys/")
